...
\begin{itemize}
    \item \textbf{OpenCV (\texttt{opencv-python}):} Utilizada para la lectura a bajo nivel y la conversión inicial de espacios de color.
    \item \textbf{NumPy (\texttt{numpy}):} Permite representar la imagen como matrices bidimensionales y realizar transformaciones complejas de manera vectorial.
    \item \textbf{PySide6 (\texttt{PySide6}):} Framework de desarrollo de la interfaz gráfica, utilizada para construir toda la ventana interactiva y los controles.
    \item \textbf{Matplotlib (\texttt{matplotlib}):} Herramienta para el cálculo y renderizado gráfico de los histogramas.
\end{itemize}
...
\subsubsection{Descripción de la imagen de Entrada}
...
\subsubsection{Evaluación de la Distribución de Intensidades Iniciales}
...
\subsubsection{Optimización del Rango Dinámico mediante Normalización}
...
\subsubsection{Impacto de la Redistribución en el Histograma}
...
\subsubsection{Estudio Comparativo de Mejora del Contraste}
...
\subsubsection{Evaluación Espectral Individual por Canal RGB}
...
\subsubsection{Procesos de Abstracción: Reducción y Segmentación}
...
\subsubsection{Resumen Visual del Procesamiento Completo}
